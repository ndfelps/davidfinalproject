...
module.exports = React.createClass ({
    ...
    returnGuides: function(e) {
		...
		for(var i = 0; i<searchVal.length; i++) {
			...
			var weirdNames = searched.split('\'');
			if (weirdNames.length === 2) {
				searched = weirdNames[0].concat(weirdNames[1]);
			}
			guides.on("value", function(snapshot) {
	        guideCount = snapshot.val();
	        for(var key in guideCount) {
	        	var champ = guideCount[key].champion;
	    	...
	    	if(result.length === 0) {
				...
				for (var i = 0; i<result.length; i++) {
					$('.searchResults').append("<div class = 'guideLink'><a href = '#guide/" + result[i].id + "'><img class = 'searchChamp' src = '" + icons[result[i].champion] + "'></img>" + "<div class = 'searchTitle'>" + result[i].title + "<div class = 'searchUser'> by " + result[i].user + "</div><div class = 'searchRole'>Role: " + result[i].role +  "</div><div class = 'searchRating'> Score: " + result[i].score + "</div></div></a></div>");
				}
			}
